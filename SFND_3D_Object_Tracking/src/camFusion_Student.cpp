...
void clusterKptMatchesWithROI(BoundingBox &boundingBox, std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr, std::vector<cv::DMatch> &kptMatches)
{
    ...
    for (auto it = kptMatches.begin(); it != kptMatches.end(); it++)
    {
        ...
        {
            mean += cv::norm(curr_keypoint.pt - prev_keypoint.pt);
            size += 1;
        }
    }
    mean = mean / (size*1.0);
  ...
  for (auto it = kptMatches.begin(); it != kptMatches.end(); it++)
    {
        ...
        if (boundingBox.roi.contains(curr_keypoint.pt))
        {
            double curr_dist = cv::norm(curr_keypoint.pt - prev_keypoint.pt);

            if (curr_dist < mean*1.3)
            {
                boundingBox.keypoints.push_back(curr_keypoint);
                     ...
                     std::vector<LidarPoint> &lidarPointsCurr, double frameRate, double &TTC)
{
  ...
  curr_x = curr_x / (curr_size *1.0);
  prev_x = prev_x / (prev_size *1.0);

  double dt = 1/frameRate;
  
  
    TTC = (curr_x * dt) / (prev_x - curr_x);
  ...
  std::multimap<int, int> m {};
    ...
    for (auto match : matches) {
      ...
      for (auto b : prevFrame.boundingBoxes) {
            if (b.roi.contains(prevKp.pt)) prev_boxID = b.boxID;
        }

        for (auto b : currFrame.boundingBoxes) {
            if (b.roi.contains(currKp.pt)) curr_boxID = b.boxID;
        }
        

      m.insert({curr_boxID, prev_boxID});

        max_prev_boxID = std::max(max_prev_boxID, prev_boxID);
    }
    ...
    for (int j : currFrameBoxIDs) {

        auto rangePrevBoxIDs = m.equal_range(j);

        std::vector<int> counts(max_prev_boxID + 1, 0);
      ...
      for (auto it = rangePrevBoxIDs.first; it != rangePrevBoxIDs.second; ++it) {
        ...
        bbBestMatches.insert({modeIndex, j});
    }
